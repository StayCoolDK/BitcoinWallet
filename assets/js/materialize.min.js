...
var sRate; //We just need to get the rate once per request.
$.getJSON( "https://api.coinmarketcap.com/v2/ticker/1/", function( jData ) {
  sRate = jData.data.quotes.USD.price;
});

//Take the balance shown and input it into the inputfield
$(document).on('click', ".useMax", function(){
  var sBalanceValue = $(".totalBalance").text()
  //alert(sBalanceValue);
  $(".btcAmount").val(sBalanceValue);
  $("#btcLabel").addClass("active");

  var sBTCValue = $(".btcAmount").val();
  var sTotal;
  var sRound;

  sTotal = sBTCValue * sRate;
  sRound = sTotal.toFixed(2);
  $(".usdValue").text(sRound);
})

//Calculate btc to usd based on the BTC rate and amount.
$(document).on('change', ".btcAmount", function(e){

  var sBTCValue = $(this).val();
  var sTotal;
  var sRound;
  ...
  $(document).on("submit", ".btcAddress", function(e){
    e.preventDefault();
    var sBech32 = $("#bech32").attr('checked');
    var sP2SH = $("#p2sh").attr('checked');
    var sLegacy = $("#legacy").attr('checked');

    if($("#bech32").is(":checked")){
      $.ajax({
        url: '/wallet/create/bech32',
      }).done(function(data){
  ...
  //Pre-confirmation of transaction, to make sure that information is correct.
  $(document).on("submit", ".btcTransaction", function(e){
    e.preventDefault();
    var sAddress = $(".btcAddress").val();
    var sAmount = $(".btcAmount").val();
    var sToken = $("#csrftoken").val();
    ...
    swal({
      ...
      html: "Transactions on Bitcoin's blockchain are irreversable. Please confirm that the following information about the transaction is correct:<br><br> Address: <b>"+sAddress+"</b> <br> Amount: <b>"+sAmount+"฿</b>",
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, send!',
      cancelButtonText: 'No, cancel!',
      confirmButtonClass: 'btn green btn-success btnMargin',
      cancelButtonClass: 'btn red btn-danger btnMargin',
      ...
      if (result.value) {
        ...
        $.ajax({
          url: 'wallet/send/'+sAddress+'/'+sAmount+'/'+sToken,
        }).done(function(data){
          if(data == "Amount is over balance."){
            swal({
              title: data,
              text: "You cannot send more than you have.",
              type: "warning",
              button: "OK",
            });
          }
          else if(data == "You cannot send to your own addresses."){
            swal({
              title: data,
              text: "It's not possible to send to your own addresses.",
              type: "warning",
              button: "OK",
            });
          }
          else if(data == "Amount has to be positive."){
            swal({
              title: data,
              text: "It's not possible to send an amount below or equal to 0.",
              type: "warning",
              button: "OK",
            });
          }
          else if (data == "Address is not valid."){
            swal({
              title: data,
          ...
          else if(data == "CSRF token does not match."){
            swal({
              title: data,
              text: "The CSRF token in the form does not match the value submitted.",
              type: "warning",
              button: "OK",
            });
          }
...
$(document).ready(function(){
  ( function() {
    ...
    for (var i = 0; i < youtube.length; i++) {
      ...
      var source = "https://img.youtube.com/vi/Gc2en3nHxA4/sddefault.jpg";
  ...
  //rel=0&iv_load_policy=3&modestbranding=1&loop=1&cc_lang_pref={{app.request.locale}}&cc_load_policy=1
                this.innerHTML = "";
  ...
  $('#modal1').modal();
  $('#feedback').modal();
  $('#inputfeedback').characterCounter();
  $('.sidenav').sidenav({
    draggable: true
  });
  $('.tooltipped').tooltip();
  ...
  $(document).on("click", "#copyInputbtn", function(){
    copyToClipboard($(this).parent().siblings("#copyInput").val());
  })

  $(document).on("submit", ".sendfeedback", function(e){
    e.preventDefault();
    var sMessage = $("#inputfeedback").val();
    var iRating = $("#rating").val();

    if(sMessage.length < 50){
      swal({
        type: 'error',
        title: 'Feedback is under 50 characters',
        text: 'The feedback should be atleast 50 characters long. Please provide additional feedback.',
      })
    }
    else {
    $.ajax({
      url: '/wallet/feedback/'+sMessage+'/'+iRating,
    }).done(function(data){
      if(data == "success"){
        $("#inputfeedback").val("");
        swal({
          type: 'success',
          title: 'Thank you!',
          text: 'Thank you for providing us with feedback to improve our service.',
          footer: '<a href="#">How can i support your service?</a>',
        })
      }
    });
  }
  ...
  $(document).on("click", ".disagreeterms", function(){
